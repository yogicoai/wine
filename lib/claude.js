...
export function parseJson(text) {
  ...
  try {
    return sanitize(JSON.parse(candidate));
  } catch {
    return sanitize(repairJson(text.slice(start)));
  }
}
...
export async function callClaude(input) {
  ...
  // 웹 검색은 기본 OFF — 검색 결과가 입력 토큰의 대부분(≈76k)을 차지해 스캔 원가가 7.5배(63원→474원),
  // 소요 시간이 6배(30초→179초)로 늘어난다. 가격 정보는 무료인 네이버쇼핑 조회가 이미 담당하므로 중복.
  // 켜는 방법: 요청에 { web: true } (결과 화면의 "최신 정보 더 찾기") 또는 환경변수 WEB_SEARCH=1
  const webEnabled =
    (input.web === true || process.env.WEB_SEARCH === "1") &&
    input.web !== false &&
    !webSearchBroken;
