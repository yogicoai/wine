...
import { lowestPrice, pickRepresentative } from "./naver";
import { priceOf, hasPriceKey, priceReadyFor } from "./danawa";
...
export async function checkDeals({ max = 12, ignoreStale = false } = {}) {
  ...
  // 값을 못 가져오는데 도는 것은 뜻이 없다. 조용히 멈춘다.
  if (!hasPriceKey()) return { deals: [], checked: 0, skipped: "noApi" };
  ...
  for (const doc of targets) {
    try {
      ...
      // 네이버 쇼핑이 닫힌 뒤로 값은 다나와에서 온다 (lib/danawa.js).
      // 잘 맞는 주종에서만 부른다 — 절반도 못 찾는 주종에서 알림을 약속하면
      // 오지 않는 것을 기다리게 된다.
      const items = (await priceOf(doc.searchKeyword || doc.name, { category: doc.category })) || [];
