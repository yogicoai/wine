...
export async function saveSubscription(subscription) {
  ...
  await db.collection("push_subs").updateOne(
    ...
    {
      // 어느 앱의 구독인지 남긴다 — 와인 특가가 맥주 앱 구독자에게 가면 안 된다
      $set: { ...subscription, app: APP.key, updatedAt: new Date() },
...
export async function subscriptionCount() {
  const db = await getDb();
  if (!db) return 0;
  return db.collection("push_subs").countDocuments({ app: APP.key });
}
...
export async function sendToAll(payload) {
  ...
  // 앱 표시가 없던 시절의 구독은 와인 앱 것으로 본다 (그때는 와인뿐이었다)
  const mine = APP.key === "wine" ? { $or: [{ app: "wine" }, { app: { $exists: false } }] } : { app: APP.key };
  const subs = await db.collection("push_subs").find(mine).toArray();
  let sent = 0;
  let removed = 0;
