...
function toStub(item) {
  // 검색어로 정해진 주종이 틀릴 때가 있다 — "와인" 검색에 막걸리가 딸려 온다
  const category = recategorize(item.name) || item.category;
  const type = QUERY_TYPE[item.query] ?? null;
  const country =
    QUERY_COUNTRY[item.query] ??
    guessCountry({ name: item.name, category, type });
  // 맛 축이 없으면 취향 추천 후보에서 아예 빠진다. 품종을 알면 큰 틀은 짐작할 수 있다.
  const tasteProfile = guessTasteProfile({ name: item.name, type, category });
...
export async function harvestCatalog({ categories = ["wine"], perQuery = 20, pages = 1, confirm = false } = {}) {
  ...
  // 중복 검사는 앱 범위와 무관하다 — 사케 앱이 수확해도 이미 있는 와인을 또 넣으면 안 된다
  const index = await catalogNameIndex({ all: true });
