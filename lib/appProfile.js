...
export function ownScope(categoryField = "category", legacyField = null) {
  const or = [{ [categoryField]: { $in: APP.scope } }, { app: APP.key }];
  if (legacyField) {
    or.push({
      app: { $exists: false },
...
// 머리말에 쓰는 이름과 한 줄 소개 — 화면 언어를 따른다.
// 영문명을 워드마크처럼 쓰면 멋있어 보이지만, 바로 아래 본문이 한국어라
// 같은 화면에 두 언어가 섞인다. 사용자는 그걸 오류로 읽는다.
export const BRAND_NAME = APP.locale === "en" ? APP.nameEn : APP.name;
export const BRAND_MOTTO = APP.locale === "en" ? APP.motto : APP.mottoKo || APP.motto;
...
export function themeCss() {
  ...
  return `:root{
--gold:${g};--gold-hi:${mix(g, 255, 0.35)};--gold-lo:${mix(g, 0, 0.32)};--gold-soft:${alpha(g, 0.12)};
--wine:${w};--tint:${w};--wine-glow:${alpha(w, 0.16)};--gold-glow:${alpha(g, 0.05)};
