 ...
 * 이 앱이 내놓는 화면 언어.
 *
 * MULTILINGUAL 은 여섯 앱 전체를 한꺼번에 여는 스위치라, 술 정보 번역이 다
 * 끝나기 전에는 켤 수 없다. 그런데 앱마다 사정이 다르다 — 전통술 앱은 처음부터
 * 한국어와 영어를 함께 내놓아야 하는 앱이다.
 *
 * 그래서 앱이 제 목록을 가질 수 있게 했다. 적지 않으면 제 언어 하나뿐이고,
 * 전체 스위치가 켜지면 그때는 셋 다 열린다.
 */
export const APP_LOCALES = APP.locales?.length ? APP.locales : [APP.locale];
