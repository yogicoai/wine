...
const GLASS = {
  ...
  shot: {
    name: "샷 글라스",
    shape: "한 모금 크기의 바닥이 두꺼운 잔",
    why: "차게 얼린 술을 단숨에 — 두꺼운 바닥이 온도를 지킵니다",
    shopKeyword: "샷글라스",
  },
  highballGlass: {
    name: "하이볼 잔",
    shape: "길고 곧게 선 잔",
    why: "탄산이 오래 살고, 얼음이 세로로 쌓여 천천히 녹습니다",
    shopKeyword: "하이볼잔",
  },
  ginBalloon: {
    name: "코파 잔",
    shape: "볼이 크고 둥근 스페인식 잔",
    why: "얼음을 가득 채워도 향이 볼에 모입니다 — 진토닉의 표준 잔입니다",
    shopKeyword: "진토닉 잔",
  },
};
...
export function glasswareFor(result) {
  ...
  switch (result.category) {
    ...
    case "brandy":
      ...
      break;
    case "vodka":
      keys = ["shot", "highballGlass"];
      tip = "스트레이트는 얼린 샷 글라스, 토닉에 섞을 땐 하이볼 잔입니다.";
      break;
    case "gin":
      keys = ["ginBalloon", "highballGlass"];
      tip = "얼음은 클수록, 많을수록 좋습니다 — 덜 녹아 덜 희석됩니다.";
      break;
    case "rum":
      // 숙성 럼은 향을 보는 술, 화이트 럼은 섞는 술
      keys = /다크|골드|아네호|숙성|XO|솔레라|엑스트라/.test(textOf(result))
        ? ["snifter", "whiskyGlass"]
        : ["highballGlass"];
      tip = "숙성 럼은 위스키처럼 니트로, 화이트 럼은 칵테일로 마십니다.";
      break;
    case "tequila":
      keys = /레포사도|아네호|엑스트라/.test(textOf(result)) ? ["whiskyGlass"] : ["shot"];
      tip = "숙성 데킬라는 향을 볼 잔이 필요합니다. 소금·라임은 블랑코의 문법입니다.";
      break;
    case "baijiu":
      keys = ["sojuGlass"];
      tip = "백주는 향이 강해 아주 작은 잔으로 조금씩 마십니다.";
      break;
    case "liqueur":
      keys = ["shot", "copita"];
      tip = "달고 진한 술은 작은 잔에 조금씩 — 얼음 위에 부어도 좋습니다.";
      break;
    case "highball":
    case "rtd":
      keys = ["highballGlass"];
      tip = "캔째보다 차게 둔 잔에 따라야 탄산과 향이 삽니다.";
      break;
    default:
