 ...
 */
// category 를 받지 않으면 막걸리·위스키까지 후보에 들어온다. 와인 앱이므로 기본은 wine.
export async function recommendByTaste({ answers = null, limit = 8, band = null, category = "wine" } = {}) {
 ...
 */
export async function recommendForBeginner({ limit = 8, category = "wine", band = null } = {}) {
  const db = await getDb();
  if (!db) return null;
