...
import { APP_CATEGORIES, ownScope } from "./appProfile";

// 주종은 하나일 수도 여럿일 수도 있다 (스피리츠 앱은 브랜디·진·럼…을 함께 본다).
// 문자열이든 배열이든 받아 DB 조건과 목록으로 각각 바꿔 쓴다.
function asList(category) {
  if (!category) return APP_CATEGORIES;
...
/**
 * 왜 추천했는지 — 근거 없는 추천은 신뢰를 얻지 못한다.
 *
 * 여기서 한국어 문장을 완성해 보내면 번역을 거칠 방법이 없다. 실제로
 * 일본어 화면에서 이 한 줄만 한국어로 남아 있었다. 조각으로 돌려주고
 * 문장은 화면에서 그 언어의 어법에 맞게 조립한다 (조사도 거기서 붙인다).
 */
function reasonFor(want, have, axes) {
  ...
  return {
    kind: "axis",
    axis: best.axis,
    level: best.value >= 66 ? "high" : best.value >= 40 ? "mid" : "low",
  };
}
...
async function loadCandidates(db, { category = null, band = null, requireTaste = true } = {}) {
  ...
  query.category = catFilter(asList(category));
...
async function ownedKeys(db) {
  const docs = await db
    .collection("cellar")
    .find(ownScope("category"), { projection: { name: 1, vintage: 1 } })
    .limit(1000)
    .toArray();
...
function diversify(scored, limit) {
  ...
  // 주종이 여럿인 앱(스피리츠·전통술)에서는 한 주종이 목록을 독차지하지 않게
  // 주종끼리도 돌아가며 고른다. 주종이 하나면 줄이 하나뿐이라 예전과 같다.
  const byCategory = new Map();
  for (const s of scored) {
    const c = s.doc.category || "?";
 ...
 * 취향 맞춤 추천
 *
 * 주종마다 축도 다르고 문답도 다르므로, 앱이 여러 주종을 보면 주종별로 따로 재고
 * 마지막에 한 줄로 세운다 (스피리츠 앱의 브랜디와 진은 겹치는 축이 없어 같이 잴 수 없다).
 *
 * @param {object} opts
 * @param {Record<string, {axes, novice, maxBand, prefer, answered}>} opts.answersByCategory 주종별 문답 결과
 * @param {string|string[]|null} opts.category 없으면 이 앱이 보는 주종 전부
 */
export async function recommendByTaste({
  answersByCategory = {},
  limit = 8,
  band = null,
  category = null,
  ...
  // 기록에서 뽑은 취향 — 이 앱의 셀러만 본다.
  // (와인에 매긴 별점이 맥주 추천의 근거가 되면 안 된다)
  const cellar = await db
    .collection("cellar")
    .find(
      { rating: { $gt: 0 }, ...ownScope("category") },
      { projection: { rating: 1, tasteProfile: 1, category: 1 } }
    )
    .limit(500)
    .toArray();
  ...
  const plans = new Map();
  for (const c of cats) {
    const answers = answersByCategory[c] || null;
    const fromAnswers = answers?.axes ? axisMap(answers.axes) : null;
    if (!fromAnswers && !fromHistory) continue;
    ...
    plans.set(c, { axes, want, answers });
  }
  // 취향을 알 방법이 하나도 없다 — 화면에서 문답으로 안내한다
  if (!plans.size) return null;

  const [candidates, owned, ratings] = await Promise.all([
    loadCandidates(db, { band, category: [...plans.keys()] }),
    ownedKeys(db),
    ratingMap(db),
  ]);
  ...
  for (const doc of candidates) {
    const plan = plans.get(doc.category);
    if (!plan) continue;
    const { axes, want, answers } = plan;
    ...
    const bonus =
      ...
      (estimated ? -0.04 : 0.04) +
      (doc.tier === "full" ? 0.05 : 0) + // 눌렀을 때 읽을 내용이 있는 쪽
      (doc.image ? 0.01 : 0);
  ...
  // 화면에 보여 줄 "내 취향" 막대는 기본 주종의 것으로 한다
  const primary = plans.get(cats[0]) || plans.values().next().value;
  const answered = Math.max(0, ...cats.map((c) => answersByCategory[c]?.answered || 0));
  ...
  return {
    ...
    want: primary.axes
      .filter((a) => primary.want[a] != null)
      .map((a) => ({ axis: a, value: Math.round(primary.want[a]) })),
 ...
 */
export async function recommendForBeginner({ limit = 8, category = null, band = null } = {}) {
  const db = await getDb();
  if (!db) return null;
...
// 고정 문장이라 그대로 번역 열쇠가 된다 — 화면에서 t() 를 거친다
 ...
 */
export async function recommendByBand(band, { limit = 8, category = null } = {}) {
  const db = await getDb();
  if (!db) return null;
  ...
  //
  // 뼈대(이름만 수확해 둔 것)에도 수확 때 짐작한 맛 축이 붙어 있어 맛 축 유무로는
  // 갈리지 않는다. tier 로 가른다 — 눌렀을 때 읽을 내용이 있는 쪽을 앞에 세운다.
  const scored = candidates
    .map((doc) => {
      ...
      return {
        ...
        sort:
          (r?.avg || 0) * 100 +
          (beginnerScore(doc) ?? 0) +
          (doc.tier === "full" ? 60 : 0) + // 정식 분석을 앞에
          (doc.result?.tasteEstimated ? -15 : 0) + // 품종에서 짐작한 축은 뒤로
          (doc.image ? 5 : 0),
      };
    })
    .sort((a, b) => b.sort - a.sort);
