...
// 네이버가 검색 API 체계를 통째로 옮겼다 (2026년 7월). 공식 공지 32530.
//
//   개발자센터(developers.naver.com)  →  NAVER API HUB (네이버 클라우드 플랫폼)
//   1단계  API HUB 출시
//   2단계  2026-07-31  개발자센터에서 신규 신청 차단
//   3단계  2027-06-30  개발자센터 지원 종료. 이후 API HUB 에서만 이용 가능
//
// 문제는 이관 목록이다. API HUB 가 제공하는 것은
//   검색   블로그 · 이미지 · 웹 · 뉴스 · 백과 · 카페 · 지식iN
//   데이터랩 검색어 트렌드 · 쇼핑 "인사이트"
// 쇼핑 "검색"은 여기 없다. 인사이트는 클릭 추이 통계이지 상품·가격이 아니다.
//
// 실측도 같은 그림이다. 같은 키로 이미지·블로그·뉴스·카페·지식iN·백과·웹문서·
// 지역은 200 이 오고, shop 과 book 만 404 SE05 다. 이관 목록에 없는 둘이
// 정확히 죽어 있다. 즉 쇼핑 검색은 옮겨간 것이 아니라 없어진 것이다.
//
// v2 경로는 {"error_code":"052","message":"v2 : Partner does not exists."} 를
// 돌려주는데, 있지도 않은 이름(v2/search/zzzz)에도 같은 답이 온다. 파트너
// 확인이 주소 확인보다 먼저라 이 응답으로는 아무것도 알 수 없다.
//
// 우리가 할 일 — 이미지 검색은 2027-06-30 전에 API HUB 로 옮겨야 한다.
// 종량제이고 지금은 한시적 무료다. 쇼핑 검색은 기다릴 이유가 없다.
//
