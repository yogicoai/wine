...
// 네이버가 v1 검색 API에서 쇼핑(과 책)을 내렸다. 같은 키로 이미지·블로그·뉴스·
// 지식iN·백과·웹문서·카페·지역은 그대로 200이 오고 shop 과 book 만 404 SE05 다.
// 키를 새로 발급해도 살아나지 않는다 — 앱 등록 문제가 아니다.
//
// v2 경로(openapi.naver.com/v2/search/...)는 살아 있으나 다른 문으로 막혀 있다.
//   {"error_code":"052","message":"v2 : Partner does not exists."}
// 다만 없는 이름(v2/search/zzzz)에도 같은 답이 오므로, 파트너 확인이 주소 확인보다
// 먼저다. 즉 이 응답만으로는 v2 에 쇼핑이 있는지 없는지 알 수 없다.
// 확인하려면 네이버에 직접 물어야 한다 (dl_naver_openapi@navercorp.com).
//
