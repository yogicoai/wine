 ...
 * 사진 주소를 https 로 맞춘다.
 *
 * 이미지 검색은 사진 주소를 http://shopping.phinf.naver.net/… 으로 준다.
 * 우리 화면은 https 라, 그대로 붙이면 브라우저가 혼합 콘텐츠로 막아 버린다 —
 * 깨진 그림표만 남는다. 전통술 앱의 느린마을 막걸리가 그랬다.
 *
 * 그 호스트는 https 를 아예 열어 두지 않아(연결 실패) 앞글자만 바꿔서는 안 된다.
 * 같은 사진이 shopping-phinf.pstatic.net 에도 있고 그쪽은 https 로 열린다.
 * 카탈로그에 이미 들어 있던 3,705건도 모두 그 호스트다.
 */
export function httpsImage(url) {
  ...
  return (
    s
      // 가격비교 카탈로그 사진
      .replace(/^https?:\/\/shopping\.phinf\.naver\.net\//i, "https://shopping-phinf.pstatic.net/")
      // 개별 판매자 사진. 숫자가 붙어 나오지만(shop1, shop2…) https 쪽에는
      // 숫자 없는 호스트 하나뿐이다 — shop1-phinf.pstatic.net 은 존재하지 않는다.
      .replace(/^https?:\/\/shop\d*\.phinf\.naver\.net\//i, "https://shop-phinf.pstatic.net/")
  );
}
...
export async function searchProductImage(q, { display = 100, debug = false } = {}) {
  ...
  const shop = (data.items || [])
    .map((it) => ({ image: httpsImage(it.link), title: String(it.title || "").replace(/<[^>]+>/g, "") }))
...
// ── 안주·잔·도구 사진 ───────────────────────────────────────
// 술은 온라인으로 못 팔지만 안주와 잔과 도구는 아무 제한이 없다. 그래서 이쪽
// 링크는 실제로 구매까지 이어진다 — 앱에서 유일하게 그런 자리다.
//
// 다만 쇼핑 검색 API가 내려가 사진과 값이 함께 사라졌다. 이름만 적힌 줄에
// "쿠팡에서 검색" 링크 하나가 달려 있을 뿐이었다.
//
// 사진은 이미지 검색으로 되살릴 수 있다. 술에 쓴 것과 같은 방법이되 거르는
// 기준이 다르다. 술 쪽은 "이 술이 맞는가"를 깐깐하게 봤지만(다른 술 사진이
// 붙으면 거짓 정보가 된다), 안주는 "훈제연어" 사진이 어느 집 것이든 상관없다.
// 대신 술 사진이 안주 자리에 붙지 않게만 막는다.
//
// 상품 번호(main_)는 기대하지 않는다. 식품은 가격비교 카탈로그가 아니라 개별
// 판매자 상품으로 올라와서 그 번호가 거의 없다 — 실측으로 30건 중 0~5건이었다.
// 구매는 쿠팡 검색으로 보낸다. 식품은 정말로 살 수 있으므로 그것으로 충분하다.

// 술이라는 표시. 안주를 찾는데 술 사진이 붙으면 안 된다.
const LOOKS_LIKE_DRINK =
  /\d+(\.\d+)?\s*도(?![시자])|(750|700|720|375|500)\s*ml|와인|위스키|사케|맥주|소주|보드카|진\s|럼|데킬라|브랜디|리큐르/i;

/**
 * 안주·잔·도구 사진 한 장.
 *
 * @param {string} q 찾을 말 ("훈제연어", "와인잔")
 * @param {boolean} avoidDrink 술 사진을 걸러낸다 (안주에는 켜고, 잔·도구에는 끈다 —
 *                             "위스키 잔"처럼 술 이름이 든 물건이 있다)
 */
export async function searchGoodsImage(q, { avoidDrink = true, display = 40 } = {}) {
  if (!hasNaverKeys() || !q) return null;
  const req = searchRequest("image", `query=${encodeURIComponent(q)}&display=${display}&sort=sim`);
  const res = await fetch(req.base, { headers: req.headers, next: { revalidate: 604800 } });
  ...
  for (const it of data?.items || []) {
    ...
    // 찾는 말이 통째로 들어 있거나, 낱말이 대부분 들어 있어야 그 물건이다
    const hit = t.includes(want) || (words.length > 1 && words.filter((w) => t.includes(compact(w))).length >= words.length - 1);
    if (!hit) continue;
    if (avoidDrink && LOOKS_LIKE_DRINK.test(title)) continue;
    return { image, title };
  }
  return null;
}
