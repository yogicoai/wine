...
//   jpyLow: 1800, jpyHigh: 3200,      // 일본 현지가 범위 (엔, 참고 · 사케)
...
export function globalLabel(info) {
  // 사케는 엔화가 자연스럽다. 일본이 본토라 값이 촘촘하고, 달러로 옮기면
  // 두 번 환산하는 셈이라 오차만 커진다.
  const jl = Number(info?.jpyLow);
  const jh = Number(info?.jpyHigh);
  if (jl || jh) {
    const y = (n) => `¥${Number(n).toLocaleString("ja-JP")}`;
    if (!jh || jl === jh) return y(jl || jh);
    if (!jl) return y(jh);
    return `${y(jl)}~${y(jh)}`;
  }
