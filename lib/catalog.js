...
import { scopeQuery, inScope, APP } from "./appProfile";
...
// ── 다국어 ──────────────────────────────────────────────
// 한국어 원문(result)이 기준이다. 번역은 문서의 i18n.{en,ja} 에 글 필드만 겹쳐 둔다.
// 맛 축·키·가격대·이미지는 언어와 무관하므로 번역층에 두지 않는다.
// (i18n 은 seed 재적재가 건드리지 않는 최상단 필드라 번역이 증발하지 않는다)
...
// similar 는 번역하지 않는다 — 카탈로그의 다른 항목 이름과 이어지는 연결 고리라
// 번역하면 유사주 카드의 상호 링크가 끊긴다.
const TEXT_ARRAYS = ["tips"]; // 문자열 배열
// specs/history/foodPairing 은 구조를 유지한 채 통째로 겹친다

/**
 * 요청한 언어의 번역층을 결과에 겹친다. 번역이 없는 필드는 원문이 그대로 남는다
 * — 반쯤 번역된 항목도 화면이 깨지지 않는다.
 */
export function localizeResult(result, i18n, locale) {
  if (!locale || locale === "ko" || !i18n?.[locale]) return result;
  const layer = i18n[locale];
  const out = { ...result };
  for (const f of TEXT_FIELDS) if (layer[f] != null) out[f] = layer[f];
  for (const f of TEXT_ARRAYS) if (Array.isArray(layer[f]) && layer[f].length) out[f] = layer[f];
  for (const f of ["specs", "history", "foodPairing"]) {
    if (Array.isArray(layer[f]) && layer[f].length) out[f] = layer[f];
  }
  return out;
}
...
/**
 * 이 요청이 원하는 술 정보 언어.
 * 내 정보에서 고른 선택(쿠키 bl_locale)이 앱 기본값(APP.locale)보다 우선한다.
 * 요청 밖(스크립트·빌드)에서 불리면 next/headers 가 없거나 실패하므로 조용히 기본값으로.
 */
async function requestLocale() {
  try {
    const { cookies } = await import("next/headers");
    const store = await cookies();
    const v = store.get("bl_locale")?.value;
    if (v === "ko" || v === "en" || v === "ja") return v;
  } catch {
    // 요청 컨텍스트가 아니다 — 앱 기본값으로
  }
  return APP.locale;
}
...
export async function lookupCatalog(name, vintage, { allowStub = false } = {}) {
  ...
  // 사용자가 고른 언어(쿠키) > 앱 기본 언어 순으로 번역층을 겹친다 (한국어는 원문 그대로)
  const localized = localizeResult(found.result, found.i18n, await requestLocale());

  return {
    // 가격대는 문서 최상단(큐레이션)에 산다 — 화면이 쓰는 result 에 얹어 준다.
    // 이게 빠지면 카탈로그로 연 술은 "예상 가격" 카드가 통째로 사라진다.
    result: {
      ...localized,
      priceBand: found.result.priceBand ?? found.priceBand ?? null,
      // 시세도 문서 최상단에 산다 — 화면이 쓰는 result 에 얹어 준다
      priceInfo: found.priceInfo ?? null,
      // 일본 현지 맛 좌표·인기 순위 (さけのわ). 사케 중에서도 맞춘 것에만 있다.
      sakenowa: found.sakenowa ?? null,
    },
...
export async function saveCatalog(result, { usedWeb = false, model = null, source = "scan" } = {}) {
  ...
  // 시세 — 실시간 판매가가 아니라 우리가 조사해 둔 시장가 범위다(lib/priceInfo.js).
  // 네이버 쇼핑 검색이 문을 닫은 뒤로 "이 술이 얼마짜리인가"에 답하는 것은 이 값이다.
  // 큐레이션과 같은 성격이라 문서 최상단에 두고, seed 재적재에도 살아남는다.
  if (result.priceInfo) set.priceInfo = result.priceInfo;
...
// 범위를 씌운 것과 씌우지 않은 것을 따로 담는다. 화면이 쓰는 색인은 이 앱의
// 주종만 담아야 하고, 수확은 중복을 가리려고 카탈로그 전체를 봐야 한다.
// 한 칸에 담으면 먼저 부른 쪽이 뒤엣것을 덮어쓴다.
let indexCache = { scoped: { at: 0, rows: null }, all: { at: 0, rows: null } };

export function clearNameIndexCache() {
  indexCache = { scoped: { at: 0, rows: null }, all: { at: 0, rows: null } };
}

/**
 * 이름 색인.
 *
 * 기본은 이 앱이 다루는 주종만 담는다. 사케 앱이 와인 이름까지 들고 있으면
 * 라벨과 메뉴판을 읽을 때 엉뚱한 술에 붙는다.
 *
 * @param {object}  opts
 * @param {boolean} opts.all 카탈로그 전체를 본다 — 수확의 중복 검사처럼
 *                           앱 범위와 무관한 일에만 쓴다.
 */
export async function catalogNameIndex({ all = false } = {}) {
  const slot = all ? indexCache.all : indexCache.scoped;
  if (slot.rows && Date.now() - slot.at < INDEX_TTL) return slot.rows;

  const db = await getDb();
  if (!db) return [];
  const rows = await db
    .collection("catalog")
    // searchKeyword 에는 원어 표기가 들어 있는 경우가 있다 (獺祭, Château Talbot).
    // 메뉴판을 옮긴 이름이 한 글자 어긋나도 원어는 그대로라, 이 칸이 마지막 연결 고리가 된다.
    .find(
      all ? {} : { category: scopeQuery() },
      { projection: { name: 1, vintage: 1, producer: 1, category: 1, tier: 1, searchKeyword: 1 } }
    )
    .limit(20000)
    .toArray();

  const fresh = { at: Date.now(), rows };
  if (all) indexCache.all = fresh;
  else indexCache.scoped = fresh;
  return rows;
}
 ...
 * DB만 읽으므로 비용이 들지 않는다.
 */
export async function searchCatalog(q, { limit = 20, category = null, all = false } = {}) {
  const db = await getDb();
  ...
  // 이 앱이 다루는 주종만 보여 준다.
  // 거르지 않으면 사케 렌즈에서 "와인"을 찾았을 때 카베르네 소비뇽이 줄줄이 나온다.
  // 실제로 그랬다. 주종을 콕 집어 물으면 그것을 쓰되, 앱 범위 밖이면 빈 손으로 둔다.
  if (category) query.category = inScope(category) ? category : "__밖__";
  else if (!all) query.category = scopeQuery();
...
export async function catalogStats() {
  ...
  const [total, full, fullWine, byCategory, bySource, topHits] = await Promise.all([
    ...
    // 이 앱이 다루는 주종을 먼저 채운다. 그 진척을 따로 보여 준다
    col.countDocuments({ tier: { $ne: "stub" }, category: scopeQuery() }),
