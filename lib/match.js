...
export function isWeakToken(t) {
  return WEAK.has(String(t || "").toLowerCase());
}
 ...
 * 병합에서 빼도 되는 낱말인가.
 *
 * 약한 낱말이라고 다 뺄 수는 없다. 품종과 색은 '이 술이 무엇인가'를 말해 주지는
 * 않지만 '다른 술이다'는 확실히 말해 준다. 이것까지 빼면 몬테스 알파 카베르네와
 * 몬테스 알파 샤르도네가 한 칸에 모인다. 등급·제법·이름의 뼈대만 뺀다.
 */
/**
 * 두 이름이 서로 다른 술이라고 못 박는 신호가 있는가 (품종·색·숫자).
 *
 * 판매처 상품 사진을 고를 때도 같은 판단이 필요하다. "조쉬 셀러스 카베르네"에
 * 같은 생산자의 샤르도네 사진이 붙거나 "제이콥스 크릭 리슬링"에 스파클링 로제가
 * 붙는 일이 실제로 있었다 — 생산자만 같고 품종·색이 다른 경우다.
 */
export function conflictsWith(nameA, nameB) {
  return conflicts(tokenize(nameA), tokenize(nameB));
}

export function isDroppableToken(token) {
  const w = String(token || "").toLowerCase();
  return WEAK.has(w) && !GRAPE.has(w) && !COLOR.has(w);
}
