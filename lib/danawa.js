...
const EXPRESSION_MARKS = [
  ...
  /하이볼|칵테일|리큐르/i,
  /미니어처|미니어쳐/i,
  // 사케는 이름이 조립식이다 — 양조장 + 등급 + 특수 표현.
  // 뒤쪽 한 낱말이 다르면 아주 다른 술이고 값도 크게 갈린다.
  //   "핫카이산 … 유키무로 3년 숙성" 자리에 그냥 "핫카이산 준마이다이긴죠"가 붙었다.
  //   "샤라쿠 준마이슈" 자리에 "샤라쿠 준마이 히이레"가 붙었다.
  /유키무로|설실|시보리타테|나마자케|생주|히이레|무로카|아라바시리|나카도리|시즈쿠/,
  // 등급도 마찬가지다. 쿠보타는 센주·햐쿠주·만쥬가 모두 다른 술이다.
  /다이긴조|다이긴죠/,
  /긴조|긴죠/,
  /준마이|純米/,
  /혼조조|본조조|특별본양조/,
...
const BUNDLE =
  /\d+\s*종\s*세트|세트|x\s*\d+\s*병|\d+\s*캔|\d+\s*입(?!구)|\d+\s*개입|\d+\s*병\s*묶음|기획팩|\d+\s*\/\s*\d+/;
...
export async function searchDanawa(query, { timeout = 60000 } = {}) {
  ...
  try {
    const res = await fetch(`${ENDPOINT}?${p}`, {
      // 같은 술을 여러 사람이 보면 그때마다 크레딧이 나간다. 값은 하루에도
      // 바뀌지만 여섯 시간 안에서는 거의 같으므로 그동안 묵혀 쓴다.
      // 이것이 없으면 인기 있는 술 하나가 하루에 수백 크레딧을 먹는다.
