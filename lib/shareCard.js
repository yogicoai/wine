...
import { APP, THEME } from "./appProfile";
import { t } from "./i18n";
...
// 중계는 우리 이미지 호스팅만 허용하므로, 판매처 이미지가 들어오면 여기서 막히고
// 주종 엠블럼으로 대체된다 — 남의 상품 사진이 공유 카드에 합성되지 않게 하려는 것이다.
...
export async function renderShareCard(result, thumb) {
  ...
  glow.addColorStop(0, `${result.liquidColor || THEME.wineish}55`);
  ...
  ctx.fillText(APP.nameEn.toUpperCase(), W / 2, 128);
  ...
  ctx.fillText(t(cat.label).toUpperCase(), W / 2, y);
  ctx.letterSpacing = "0px";
  ...
  ctx.fillText(t("AI 소믈리에가 읽은 라벨"), W / 2, H - 104);
  ctx.letterSpacing = "0px";
