...
// 우리 이미지 호스팅만 허용한다.
//
// 임의의 주소를 대신 불러 주면 서버가 내부망 스캐너로 악용될 수 있다는 이유도 있지만,
// 판매처 이미지를 여기로 통과시키면 남의 상품 사진을 우리 서버가 다시 내보내는 셈이 된다.
// 그건 링크가 아니라 복제에 가깝다. 판매처 이미지는 <img>로 직접 링크만 걸고
// 캔버스(공유 카드)에는 올리지 않는다.
const ALLOWED = [
  /(^|\.)cafe24\.com$/i,
  /(^|\.)cafe24img\.com$/i,
];
